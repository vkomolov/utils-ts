...
>(
  ...
  return <P extends unknown[]>(cb: (...params: P) => void, params?: P) => {
    ...
    listenerOwner.addEventListener(event as string, handler);

    return () => {
      listenerOwner.removeEventListener(event as string, handler);
    };
  };
}
